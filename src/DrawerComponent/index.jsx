...
export default function DrawerComponent({ image, setImage, setActions, Actions, setbackDrop, compressionFactor, setCompressionFactor }) {
  ...
  const getFilter = async (filter) => {
    if (!image)
      ...
      try {
        setbackDrop(true);
        var response = await axios.post('https://photoshop-sz.onrender.com/filter', { filter: filter, image: image });
  ...
  return (
    <Box sx={{ height: "90vh", overflowY: "scroll", overflowX: 'hidden' }}>
      ...
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <FilterIcon />
          <Typography variant="subtitle2">&emsp;Filters</Typography>
        </AccordionSummary>
        <AccordionDetails sx={{ overflowY: 'auto' }}>
          {filters.map((arg, index) => (
            <Box sx={{ display: "flex", alignItems: "center", cursor: "pointer", marginLeft: "1vw" }} key={index}>
              <Box><img style={{ height: "auto", width: "5vw" }} src={arg.image} alt={arg.title} /></Box>
              <Box><ListItem onClick={() => { getFilter(arg.filter) }}><Typography className='sub-category-font'>{arg.title}</Typography>&ensp;</ListItem></Box>
            </Box>
          ))}
        </AccordionDetails>
      </Accordion>
      ...
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <DeblurIcon />
          <Typography variant="subtitle2">&emsp;Tuning</Typography>
        </AccordionSummary>
        <AccordionDetails>
          {fineTune.map((arg, index) => (
            <Box sx={{ display: "flex", alignItems: "center", cursor: "pointer" }} key={index}>
              <Box><img style={{ height: "auto", width: "5vw" }} src={arg.image} alt={arg.title} /></Box>
              <Box><ListItem onClick={() => { getFilter(arg.filter) }}>{arg.title}&ensp;</ListItem></Box>
            </Box>
          ))}
        </AccordionDetails>
      </Accordion>
      ...
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <FlipIcon />
          <Typography variant="subtitle2">&emsp;Flip & Mirror</Typography>
        </AccordionSummary>
        <AccordionDetails>
      ...
      <Accordion>
        ...
        <AccordionDetails>
          <Box>
            <Typography gutterBottom>Compression Factor: {Math.round(compressionFactor * 100)}%</Typography>
            <Slider
